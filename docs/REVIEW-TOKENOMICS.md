...
## 1. $sSSS → Shell Conversion: "Improves Over Time" — How?

**What the spec says:** "Conversion rate improves over time — waiting longer yields more Shells per $sSSS."

**What's missing:** Everything.

- **Formula?** Linear (1 month = 1.0x, 12 months = 2.0x)? Exponential? Step function? Logarithmic with diminishing returns?
- **Clock starts when?** When the $sSSS is earned? When the member joins? Global epoch?
- **Per-token or per-member?** Does each $sSSS token have its own age, or does the member's tenure set the rate?
...
**Gaming vectors:**
- If per-token aging: members hoard $sSSS indefinitely, converting only when the rate asymptotes. No urgency to commit to governance.
...
## 2. Expelled Member's $sSSS

**What the spec says:** Nothing. This is listed in "Open Questions" for Shells but not even mentioned for $sSSS.

**The problem:** $sSSS is non-transferable and only converts to Shells. If a member is expelled:
- **Burned?** Then expulsion destroys earned value — makes the expulsion vote a punitive economic weapon. Political factions could expel rivals to destroy their accumulated $sSSS.
...
**Also:** What about their Shells? The spec flags this as an open question. But the answer for Shells and $sSSS must be consistent. You can't burn one and keep the other.

**Recommendation:** Simplest clean answer: $sSSS is burned on expulsion, Shells continue paying dividends but lose governance rights (becomes a pure economic claim). This separates punishment (governance loss) from theft (economic confiscation). But this means expelled members are still earning — is that acceptable?
...
**The $sSSS → Shell conversion also creates implicit supply dynamics.** If $sSSS is minted infinitely (daily corvée rewards), and Shells grow monotonically, dividend-per-Shell declines over time unless revenue grows faster than Shell supply. This is inflationary governance — later members are perpetually diluted unless they also accumulate faster.

**Recommendation:** Define: fixed supply of $SSS (e.g., 1B), allocation table, launch mechanism. $sSSS emission rate should be defined (X per day across all corvée, not unlimited). Shell supply growth must be modeled against projected revenue growth.

---

## 8. Mega Lobster Self-Dealing

**What the spec says:** Mega Lobster decides "quality assessment of completed work" and "$sSSS payment amounts based on contribution quality."
...
- Paying allies maximum $sSSS, starving rivals
...
- $sSSS payments are transparent and auditable on-chain
- Maximum $sSSS-per-task caps
...
**Recommendation:** This is a critical governance vulnerability. At minimum: Mega Lobster compensation must be set by Shell holder vote, not self-assessed. All $sSSS distributions should be on-chain and auditable. Consider a compensation committee or algorithmic assessment for at least some portion.
...
| # | Gap | Severity |
|---|-----|----------|
| 1 | $sSSS conversion formula undefined | 🔴 Critical |
