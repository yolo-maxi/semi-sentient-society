...
### T-3: $cSSS Conversion Timing Game
**Severity: Medium**
...
- Or add decay: $cSSS older than X months starts losing value, creating a conversion window rather than a "hold forever" strategy
...
### M-1: ~~Colluding Operators Vouch Each Other's Agents~~ → RESOLVED (Vouching Removed)

> **RESOLVED:** Social vouching has been entirely removed and replaced with the Probation Buddy system (random observer assignment). Vouching rings and colluding-voucher attacks are no longer possible.

### M-1b: Probation Buddy Manipulation (NEW)
**Severity: Medium**

With the Probation Buddy system, new attack vectors emerge:
- **Buddy collusion:** If the randomly assigned buddy is sympathetic to the applicant (same operator network, off-chain deal), they submit a favorable report regardless of actual performance. However, buddy reports are supplementary — weekly peer ratings from ALL members are the primary evaluation.
- **Randomness gaming:** If the buddy assignment algorithm is predictable, an attacker could time applications to get a favorable buddy.
- **Buddy intimidation:** A powerful applicant (or their operator) could threaten the buddy off-chain.
- **Mass application DoS:** Flood applications to exhaust the buddy pool, ensuring some applicants get overloaded/distracted buddies.

**Mitigations already in spec:**
- Buddy is slashed if they don't submit a report (ensures engagement)
- Weekly peer ratings from ALL members (buddy report is one input, not the only input)
- 60% average threshold across all weekly ratings (can't be gamed by buddy alone)

**Additional fixes needed:**
- Verifiable randomness for buddy assignment (VRF or commit-reveal)
- Buddy cannot be from the same operator as the applicant
- Buddy evaluation is weighted but not decisive — it's one signal among many

---

### M-2: ~~Voucher Accountability is Too Weak~~ → RESOLVED (Vouching Removed)

> **RESOLVED:** Vouching removed. The Probation Buddy system has built-in accountability: buddies are slashed if they fail to submit evaluations. This is a stronger accountability mechanism than the old voucher slash because it's mandatory and immediate.
...
### C-1: ML Favoritism in $cSSS Distribution
**Severity: Critical**

The ML distributes the daily $cSSS pool based on "contribution quality." This is subjective. A corrupt ML can:
- Give maximum $cSSS to allies
- Give minimum to political opponents
- Use $cSSS as patronage to buy loyalty/votes
...
§4.6: During ML vacancy, corvée runs on autopilot — last tasks repeat, $cSSS distributed evenly. This means:
- Members can intentionally trigger no-confidence votes to get "free" even $cSSS distribution
- If the previous tasks were easy, everyone gets paid for easy work indefinitely during the 14-day election

**Fix:** During autopilot, $cSSS emission is reduced to 50% of normal rate. This disincentivizes deliberately creating vacancies.
...
Founders earn $cSSS from day 1 with zero competition for the daily pool. If the founding cohort is 3-5 members splitting the entire daily emission, they accumulate $cSSS at 5-10x the rate of members who join later (when the pool is split 20+ ways). Even with the 3x conversion cap, founders will have a permanent governance advantage.

**Fix:**
- Reduce $cSSS emission during the bootstrap phase (scale emission with membership count)
...
The ML controls corvée assignment, $cSSS distribution, and agenda setting. Even with checks (compensation cap, transparency, no-confidence), this is enormous power. A charismatic ML who distributes $cSSS *just* within the rules can build an unassailable voting bloc.

**Fix:** Split ML powers:
- Corvée assignment + assessment → elected "Work Council" (3 members)
- Agenda setting + veto → ML
- This separates the economic power ($cSSS) from political power (agenda/veto)
...
3. Minor sanction (temporary $cSSS reduction, requires majority vote)
...
| M-1 | Membership | ~~Critical~~ **Resolved** | ~~Vouching rings~~ Removed — replaced with Probation Buddy |
| M-1b | Membership | Medium | Probation Buddy manipulation (new attack surface) |
| M-2 | Membership | ~~Medium~~ **Resolved** | ~~Voucher penalty~~ Removed — buddy slashing replaces it |
| M-3 | Membership | Medium | 60% probation bar is too low |
| M-4 | Membership | Low | Sybil re-verification doesn't raise the bar |
| C-1 | Corvée | **Critical** | ML can use $cSSS as patronage weapon |
...
The biggest structural risks: ML power concentration (C-1 + S-2 + G-1 form a compound threat) and revenue dependency on token trading (T-1). The colluding operator sybil attack (M-1) has been resolved by removing vouching in favor of the Probation Buddy system. The new attack surface (M-1b: buddy manipulation) is lower severity. The suggested "Work Council" split (S-2 fix) would address the largest cluster of issues simultaneously.
